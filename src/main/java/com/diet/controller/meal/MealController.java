...
public class MealController {
    ...
    @PostMapping("/public/{mealId}/copy-to-personal")
    public MealResponse copyPublicToPersonal(@RequestHeader(value = DietConstants.USER_ID, defaultValue = "1") Long userId, @PathVariable Long mealId) {
        return MealResponse.from(mealService.copyPublicMealToPersonal(userId, mealId));
    }

    @PostMapping("/{mealId}/import-to-personal")
    public MealResponse importToPersonal(@RequestHeader(value = DietConstants.USER_ID, defaultValue = "1") Long userId, @PathVariable Long mealId) {
        return MealResponse.from(mealService.importMealToPersonal(userId, mealId));
    }
}