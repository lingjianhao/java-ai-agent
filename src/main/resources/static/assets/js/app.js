(function () {
    ...
    function renderMealCard(meal, options) {
        ...
        const copyable = options && options.copyable;
        return `
            <article class="meal-card">
                ...
                ${feedback ? `
                    <div class="button-row">
                        ...
                        ${meal.sourceType === "PUBLIC" ? `<button class="btn primary" data-action="import-meal" data-id="${escapeHtml(meal.id)}" data-name="${escapeHtml(meal.name)}">导入个人食谱</button>` : ""}
                    </div>
                ` : ""}
                ${copyable ? `
                    <div class="button-row">
                        <button class="btn primary" data-action="copy-to-personal" data-id="${escapeHtml(meal.id)}">添加到个人库</button>
                    </div>
                ` : ""}
            </article>
        `;
    ...
    async function copyToPersonal(id, button) {
        const meal = state.publicMeals.find((item) => String(item.id) === String(id));
        if (!meal) {
            showToast("没有找到该公共餐食", "error");
            return;
        }
        if (!window.confirm(`确定将“${meal.name}”添加到个人库？`)) {
            return;
        }
        const restore = setLoading(button, "添加中...");
        try {
            await guard(async () => {
                await DietApi.copyPublicMealToPersonal(id);
                await ensurePersonalMeals(true);
                state.home.loaded = false;
            }, `“${meal.name}”已添加到个人库`);
        } finally {
    ...
    async function importMealToPersonal(id, name, button) {
        if (!window.confirm(`确定将“${name}”导入个人食谱？`)) {
            return;
        }
        const restore = setLoading(button, "导入中...");
        try {
            await guard(async () => {
                await DietApi.importMealToPersonal(id);
                state.home.loaded = false;
            }, `“${name}”已导入个人食谱`);
        } finally {
            restore();
    ...
    function renderPublicMeals() {
        app.innerHTML = `
            <section class="section">
                ...
                <div id="publicMealList">${renderMealList(state.publicMeals, { copyable: true })}</div>
            </section>
        `;
        ensurePublicMeals();
    }
    async function ensurePublicMeals(force) {
        ...
        try {
            ...
            if (currentRoute() === "/diet/meals/public") {
                document.getElementById("publicMealList").innerHTML = renderMealList(state.publicMeals, { copyable: true });
            }
        } catch (error) {
    ...
    function handleClick(event) {
        ...
        } else if (action === "copy-to-personal") {
            copyToPersonal(target.dataset.id, target);
        } else if (action === "import-meal") {
            importMealToPersonal(target.dataset.id, target.dataset.name, target);
